...
class Global {
    public:
        float width = 10.0;
        float height = 4.0;
        int winX = WINDOW_WIDTH;
        int winY = WINDOW_HEIGHT;
        //int numShelf = 3;
...
void display(void)
{
    ...
    //Create Visual Grid
    //int SizeOfGrid = 50;
    int SizeOfGrid = gl.winX/20;
    glPushMatrix();
    /*
    for (int i = 0; i < gl.winX; i++) {
        if (i % SizeOfGrid == 0) {
            for (int j = 0; j < gl.winY; j++) {
...
void movePerson(float x, float y, int p)
{
    ...
    for (int i = 0; i < NUM_PEOPLE; i++) {
        printf("Person p= %d\n", p);
        if (p == personChoice) {
            printf("Person: %d Ignored\n", i);
            continue;
        }
        printf("Person: %d\n", i);
        if (gl.person[p].pos[1] < (gl.person[i].pos[1] + 4) && 
            gl.person[p].pos[1] > (gl.person[i].pos[1] - 4)) {
                printf("collision on y deteced\n");
                if (gl.person[p].pos[0] < (gl.person[i].pos[0] + 4) &&
                    gl.person[p].pos[0] > (gl.person[i].pos[0] - 4)) {
                        printf("collision deteced\n");
                        if (x > 0.0) {
                            gl.person[p].pos[0] = gl.person[i].pos[0] + 3;
                        }
                        if (x < 0.0) {
                            gl.person[p].pos[0] = gl.person[i].pos[0] - 3;
                        }
                        if (y > 0.0) {
                            gl.person[p].pos[1] = gl.person[i].pos[1] - 3;
                        }
                        if (y < 0.0) {
                            gl.person[p].pos[1] = gl.person[i].pos[1] + 3;
                        }
